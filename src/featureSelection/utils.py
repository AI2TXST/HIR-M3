...
try:
    ...
    class FeatureDropout(nn.Module):
        ...
        def forward(self, x):
            if not self.training or self.p <= 0.0 or x.numel() == 0:
                return x
            if self.p >= 1.0:
                return torch.zeros_like(x)
            mask = (torch.rand(x.size(0), x.size(1), 1, device=x.device) >= self.p).to(x.dtype)
...
def hir_importance(X_train, y_train, X_test, y_test, epochs=2, batch_size=32):
    ...
    try:
        ...
        # Subsample training to 10,000 samples & test to 1,000 for fast, memory-safe attention ranking
        if len(X_tr_np) > 10000:
            idx_tr = np.random.RandomState(42).choice(len(X_tr_np), 10000, replace=False)
            X_tr_np, y_tr_np = X_tr_np[idx_tr], y_tr_np[idx_tr]
        if len(X_te_np) > 1000:
            idx_te = np.random.RandomState(42).choice(len(X_te_np), 1000, replace=False)
            X_te_np = X_te_np[idx_te]

        num_features = X_tr_np.shape[1]
        model = HIRModel(num_features=num_features, embed_dim=16).to(device)
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, n_samples, batch_size):
                ...
                optimizer.zero_grad()
                logits, _ = model(batch_x, need_weights=False)
                loss = criterion(logits, batch_y)
                loss.backward()
                optimizer.step()
                
        model.eval()
        total_attn = np.zeros((num_features, num_features), dtype=np.float32)
        total_count = 0
        eval_batch = 32
        with torch.no_grad():
            for i in range(0, len(X_te_np), eval_batch):
                batch_x = torch.tensor(X_te_np[i:i+eval_batch], dtype=torch.float32).to(device)
                _, attn = model(batch_x, need_weights=True)
                if attn is not None:
                    total_attn += attn.sum(dim=0).cpu().numpy()
                    total_count += len(batch_x)
                
        global_attn_matrix = total_attn / max(total_count, 1)
...
def get_m3_feature_groups(all_features):
    ...
    MICRO_FEATURES = valid([
        ...
        # ICD clusters, HICD-BERT embeddings, and hierarchy features
...
def load_and_preprocess_data(file_path_or_df, cohort_name, target='ever_readmitted'):
    ...
    # Drop obvious leakage / ID / date / post-index columns
    drops = [
        'BENE_ID', 'Beneficiary_ID', 'Assessment_Effective_Date',
        'COUNTYFIPS', 'ever_deceased', 'Days_Cared_For', 'NumVisits',
        'DaysBetweenVisits', 'PrevVisitDate', 'Last_Assessment_Date',
        'has_diabetes', 'has_heart_failure', 'has_hypertension', t_col
    ]
    ...
    cols = [clean_column_name(c) for c in X_num.columns]
    
    # Split using numpy index arrays to prevent pandas _consolidate_inplace RAM duplication
    idx_tr, idx_te = train_test_split(np.arange(len(y)), test_size=0.2, random_state=42, stratify=y)
    
    y_train = y[idx_tr]
    y_test = y[idx_te]

    # Convert to numpy for memory-efficient scaling
    X_tr_np = X_num.iloc[idx_tr].values.astype(np.float32)
    X_te_np = X_num.iloc[idx_te].values.astype(np.float32)

    del X_num
    gc.collect()

    sc = StandardScaler()
    X_tr_scaled = sc.fit_transform(X_tr_np).astype(np.float32)
    X_te_scaled = sc.transform(X_te_np).astype(np.float32)

    del X_tr_np, X_te_np
    gc.collect()

    X_train_scaled = pd.DataFrame(X_tr_scaled, columns=cols)
    X_test_scaled = pd.DataFrame(X_te_scaled, columns=cols)
