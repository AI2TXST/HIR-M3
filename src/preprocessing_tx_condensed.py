...
def get_spark_session(app_name="OASIS_Preprocessing_TX_Condensed_CPU"):
...
def preprocess_notebook_pipeline_condensed_tx(
    input_file, 
    output_file, 
    min_freq_count=200
):
    """
    Optimized and Condensed PySpark Preprocessing Pipeline for Texas OASIS Cohort (CPU).
    """
    ...
    spark = get_spark_session("OASIS_Preprocessing_TX_Condensed_CPU")
    ...
    spark.stop()
    logging.info(f"Condensed Texas preprocessing complete in {time.time() - start_time:.2f} seconds!")

def main():
    input_file = "data/final_mergedDF_TX.csv"
    output_file = "data/processed_final_mergedDF_condensed_TX.csv"
    preprocess_notebook_pipeline_condensed_tx(input_file, output_file)
