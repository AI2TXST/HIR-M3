    ...
    for epoch in range(config['EPOCHS']):
        ...
        for batch_idx, (batch_x, batch_y, batch_w) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 20 == 0 or (batch_idx + 1) == len(train_loader):
                logging.info(f"    [HIR-M3 Transformer] Epoch {epoch+1}/{config['EPOCHS']} - Batch {batch_idx+1}/{len(train_loader)} - Loss: {loss.item():.4f}")
...
def train_lgb(X_train, y_train, w_train, X_test, y_test, config=None):
...
def train_xgb(X_train, y_train, w_train, X_test, y_test, config=None):
    ...
    n_pos = float(np.sum(y_tr == 1))
    n_neg = float(np.sum(y_tr == 0))
    spw = float(n_neg / max(1.0, n_pos))

    xgb_model = xgb.XGBClassifier(
        n_estimators=100, learning_rate=0.05, max_depth=5, scale_pos_weight=spw,
        tree_method='hist', random_state=42, use_label_encoder=False, eval_metric='logloss', n_jobs=4
    )
    if config and isinstance(config, dict):
        xgb_model.set_params(**config)
    xgb_model.fit(X_tr, y_tr, sample_weight=w_tr)
    return xgb_model.predict_proba(X_test)[:, 1]

def train_cb(X_train, y_train, w_train, X_test, y_test, config=None):
    ...
    cb_model = cb.CatBoostClassifier(
        iterations=100, learning_rate=0.05, depth=6, scale_pos_weight=spw,
        random_state=42, verbose=0, thread_count=-1
    )
    if config and isinstance(config, dict):
        cb_model.set_params(**config)
    cb_model.fit(X_tr, y_tr, sample_weight=w_tr)
    return cb_model.predict_proba(X_test)[:, 1]

def train_rf(X_train, y_train, w_train, X_test, y_test, config=None):
    X_tr, X_val, y_tr, y_val, w_tr, _ = train_test_split(
        X_train, y_train, w_train, test_size=0.1, stratify=y_train, random_state=42
    )
    rf = RandomForestClassifier(n_estimators=100, max_depth=15, class_weight='balanced_subsample', random_state=42, n_jobs=-1)
    if config and isinstance(config, dict):
        rf.set_params(**config)
...
def train_gbdt(X_train, y_train, w_train, X_test, y_test, config=None):
    X_tr, X_val, y_tr, y_val, w_tr, _ = train_test_split(
        X_train, y_train, w_train, test_size=0.1, stratify=y_train, random_state=42
    )
    n_pos = float(np.sum(y_tr == 1))
    n_neg = float(np.sum(y_tr == 0))
    spw = float(n_neg / max(1.0, n_pos))
    adjusted_w_tr = w_tr * np.where(y_tr == 1, spw, 1.0)

    gbdt = GradientBoostingClassifier(n_estimators=50, max_depth=5, max_features='sqrt', random_state=42)
    if config and isinstance(config, dict):
        gbdt.set_params(**config)
    gbdt.fit(X_tr, y_tr, sample_weight=adjusted_w_tr)
    return gbdt.predict_proba(X_test)[:, 1]

def train_logreg(X_train, y_train, w_train, X_test, y_test, config=None):
    X_tr, X_val, y_tr, y_val, w_tr, _ = train_test_split(
        X_train, y_train, w_train, test_size=0.1, stratify=y_train, random_state=42
    )
    logreg = LogisticRegression(max_iter=1000, class_weight='balanced', random_state=42)
    if config and isinstance(config, dict):
        logreg.set_params(**config)
