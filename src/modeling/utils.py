...
def permutation_importance_rf(X_train, y_train, X_test, y_test):
    ...
    rf.fit(X_train, y_train)
    if len(X_test) > 5000:
        idx = np.random.RandomState(1).choice(len(X_test), 5000, replace=False)
        X_eval, y_eval = X_test.iloc[idx], y_test[idx]
    else:
        X_eval, y_eval = X_test, y_test
    perm_importance_rf = permutation_importance(rf, X_eval, y_eval, n_repeats=5, random_state=1, n_jobs=1)
...
def permutation_importance_ridge(X_train, y_train, X_test, y_test):
    from sklearn.linear_model import LogisticRegression
    ridge = LogisticRegression(penalty='l2', C=1.0, random_state=42, max_iter=1000)
    ...
    if len(X_test) > 5000:
        ...
        X_eval, y_eval = X_test, y_test
    perm_importance_ridge = permutation_importance(ridge, X_eval, y_eval, n_repeats=5, random_state=1, n_jobs=1)
...
def get_m3_feature_groups(all_features):
    ...
    MICRO_FEATURES = valid([
        ...
        # Clinical baseline & physical parameters
