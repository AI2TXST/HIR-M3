...
def get_spark_session(app_name="OASIS_Preprocessing_NB"):
...
def preprocess_notebook_pipeline(input_csv, output_csv, corr_threshold=0.7):
    ...
    # Clean any column names containing special characters (dots, spaces, hyphens)
    ...
    # 6. Save directly from PySpark to CSV on disk (0 MB driver RAM used!)
    logging.info(f"Saving processed dataset directly from PySpark to {output_csv}...")
    ...
    if part_files:
        ...
        logging.info(f"Successfully saved processed dataset directly to {output_csv}!")
    else:
...
def main():
    ...
    output_file = "data/processed_final_mergedDF.csv"
